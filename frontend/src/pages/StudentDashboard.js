...
import { GraduationCap, LogOut, CreditCard, Calendar, Clock, Users } from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  const canJoinClass = (classDate, startTime) => {
    ...
    const fiveMinutesBefore = new Date(classDateTime.getTime() - 5 * 60000);
    return now >= fiveMinutesBefore && now <= classDateTime;
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-sky-500 to-sky-600 rounded-3xl p-6 text-white shadow-[4px_4px_0px_0px_rgba(14,165,233,0.3)]">
            ...
            <Button
              ...
              className="w-full mt-4 bg-white text-sky-600 hover:bg-slate-50 rounded-full font-bold"
        ...
        <div className="mb-8">
          <Button
            onClick={() => navigate('/browse-classes')}
            className="bg-amber-400 hover:bg-amber-500 text-slate-900 rounded-full px-8 py-6 font-bold text-lg shadow-[0_4px_14px_0_rgba(245,158,11,0.39)] hover:-translate-y-0.5 transition-all"
            data-testid="browse-classes-button"
          >
        ...
        {/* Upcoming Classes */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-4">Upcoming Classes</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {upcomingClasses.map((cls) => (
                <div
                  ...
                  data-testid={`upcoming-class-${cls.class_id}`}
                >
                  <div className="p-6">
                    ...
                    <div className="flex gap-2">
                      {canJoinClass(cls.date, cls.start_time) && (
                        <Button
                          ...
                          data-testid={`join-class-button-${cls.class_id}`}
                        >
                          Join Class
                        </Button>
                      )}
                      <Button
                        onClick={() => handleCancelBooking(cls.class_id)}
                        variant="outline"
                        className="flex-1 rounded-full border-2 hover:bg-red-50 hover:border-red-200"
                        data-testid={`cancel-booking-button-${cls.class_id}`}
                      >
                        Cancel Booking
                      </Button>
                    </div>
        ...
        {/* Past Classes */}
        <div>
          <h2 className="text-2xl font-bold text-slate-900 mb-4">Past Classes</h2>
          {pastClasses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {pastClasses.map((cls) => (
                <div
                  key={cls.class_id}
                  className="bg-white rounded-3xl border-2 border-slate-100 p-6 opacity-80"
                >
