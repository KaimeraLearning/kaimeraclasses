...
export default function CounselorProfile() {
  ...
  const fetchProfile = async () => {
    try {
      const res = await apiFetch(`${API}/counsellor/profile`, { credentials: 'include' });
      ...
      setForm({
        ...
        interests_hobbies: data.interests_hobbies || '', experience: data.experience || '',
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await apiFetch(`${API}/counsellor/update-full-profile`, {
  ...
  const handleResume = async (e) => {
    ...
    reader.onload = async () => {
      try {
        const res = await apiFetch(`${API}/counsellor/upload-resume`, {
          method: 'POST', credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-100 p-6" data-testid="profile-header-card">
          <div className="flex items-start gap-6">
            <div className="relative group">
              <div className="w-28 h-28 rounded-full bg-gradient-to-br from-violet-100 to-fuchsia-100 flex items-center justify-center overflow-hidden border-4 border-white shadow-lg">
              ...
              <label className="absolute bottom-0 right-0 bg-violet-500 text-white rounded-full w-8 h-8 flex items-center justify-center cursor-pointer hover:bg-violet-600 shadow-md" data-testid="upload-profile-pic">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400 mt-1">Counselor ID: <span className="font-mono font-bold text-violet-600">{profile?.counselor_id || profile?.user_id?.slice(-8)?.toUpperCase() || 'N/A'}</span></p>
              <div className="flex items-center gap-2 mt-2">
                <Star className="w-4 h-4 text-violet-500" />
                <span className="text-sm font-semibold text-slate-700">KL-CAT Score: <span className="text-violet-600">{profile?.klcat_score || 'Not set'}</span></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-100 p-6 space-y-4" data-testid="personal-info-card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2"><Label>KL-CAT Score</Label><Input value={form.klcat_score} onChange={e => setForm({...form, klcat_score: e.target.value})} placeholder="Enter your KL-CAT score" className="rounded-xl" data-testid="profile-klcat-score" /></div>
            ...
            <div className="sm:col-span-2"><Label>Experience</Label><textarea value={form.experience} onChange={e => setForm({...form, experience: e.target.value})} className="w-full rounded-xl border-2 border-slate-200 px-3 py-2 text-sm" rows={3} data-testid="profile-experience" /></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-100 p-6 space-y-4" data-testid="resume-card">
          ...
          <div className="flex items-center gap-4">
            {profile?.resume_name ? (
              <div className="flex items-center gap-3 bg-violet-50 border border-violet-200 rounded-xl px-4 py-3 flex-1">
                <FileText className="w-5 h-5 text-violet-600" />
                <span className="text-sm font-medium text-violet-800">{profile.resume_name}</span>
            ...
            <label className="cursor-pointer" data-testid="upload-resume-btn">
              <div className="bg-violet-500 hover:bg-violet-600 text-white rounded-full px-4 py-2 text-sm font-medium flex items-center gap-2 transition-colors">
        ...
        {/* Save Button */}
        <Button onClick={handleSave} disabled={saving} className="w-full bg-violet-500 hover:bg-violet-600 text-white rounded-full py-6 font-bold text-lg" data-testid="save-profile-btn">
