...
export default function CounselorProfile() {
  ...
  const fetchProfile = async () => {
    try {
      const res = await fetch(`${API}/counsellor/profile`, { credentials: 'include' });
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await fetch(`${API}/counsellor/update-full-profile`, {
  ...
  const handleResume = async (e) => {
    ...
    reader.onload = async () => {
      try {
        const res = await fetch(`${API}/counsellor/upload-resume`, {
          method: 'POST', credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
